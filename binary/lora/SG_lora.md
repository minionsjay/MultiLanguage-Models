...
| 模型路径 | `Training/binary_8countries/SG_lora/final_model` |
| 模型大小 | 0.02 GB |
| 可训参数 | 596,745 (0.21% of 279M) |
| 数据源 | Datasets_Nine_country/Singapore/all_datasets_with_labels_dedup.csv |
...
|------|-----------|--------|------|--------|
| **clean (safe)** | 0.9511 | 0.9003 | **0.9250** | 11559 |
| **violation** | 0.6974 | 0.8324 | **0.7589** | 3192 |
...
- **clean recall = 0.9003**: 90.0% 的安全内容被正确识别
- **violation recall = 0.8324**: 83.2% 的违规内容被正确召回
- **clean precision = 0.9511**: 模型判为 clean 的内容中 95.1% 确实是 clean
- **violation precision = 0.6974**: 模型判为 violation 的内容中 69.7% 确实是违规
...
model = PeftModel.from_pretrained(
    base_model,
    "/home/ninini/Agents/Colloation_data/Training/binary_8countries/SG_lora/final_model"
)
...
# 示例
text = "some Singapore social media post..."
...
| 指标 | Full FT | LoRA | Δ |
|------|---------|------|------|
| Accuracy | 0.9185 | 0.8856 | -0.0329 |
| Macro F1 | 0.8807 | 0.8420 | -0.0387 |
| Clean F1 | 0.9479 | 0.9250 | -0.0229 |
| Viol F1 | 0.8135 | 0.7589 | -0.0545 |
| 训练参数 | 279M | 597K (0.21%) | - |
| 训练速度 | 基准 | 更快 | - |
...
- Full FT 版本: [SG_fullft.md](SG_fullft.md)（更高精度）
