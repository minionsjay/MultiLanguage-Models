...
| 模型路径 | `Training/binary_8countries/ZA_lora/final_model` |
| 模型大小 | 0.02 GB |
| 可训参数 | 596,745 (0.21% of 279M) |
| 数据源 | Datasets_Nine_country/South Africa/all_datasets_with_labels_dedup.csv |
...
|------|-----------|--------|------|--------|
| **clean (safe)** | 0.7099 | 0.7976 | **0.7512** | 5261 |
| **violation** | 0.8658 | 0.8002 | **0.8317** | 8583 |
...
- **clean recall = 0.7976**: 79.8% 的安全内容被正确识别
- **violation recall = 0.8002**: 80.0% 的违规内容被正确召回
- **clean precision = 0.7099**: 模型判为 clean 的内容中 71.0% 确实是 clean
- **violation precision = 0.8658**: 模型判为 violation 的内容中 86.6% 确实是违规
...
model = PeftModel.from_pretrained(
    base_model,
    "/home/ninini/Agents/Colloation_data/Training/binary_8countries/ZA_lora/final_model"
)
...
# 示例
text = "some South Africa social media post..."
...
| 指标 | Full FT | LoRA | Δ |
|------|---------|------|------|
| Accuracy | 0.8491 | 0.7992 | -0.0499 |
| Macro F1 | 0.8412 | 0.7914 | -0.0498 |
| Clean F1 | 0.8058 | 0.7512 | -0.0546 |
| Viol F1 | 0.8766 | 0.8317 | -0.0449 |
| 训练参数 | 279M | 597K (0.21%) | - |
| 训练速度 | 基准 | 更快 | - |
...
- **仅二分类**: 不能区分违规类型
- **单国家**: 仅针对 South Africa
- **精度低于 Full FT**: 适合快速部署，生产环境建议 Full FT

## 相关模型

- Full FT 版本: [ZA_fullft.md](ZA_fullft.md)（更高精度）
