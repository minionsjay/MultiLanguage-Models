...
| 模型路径 | `Training/binary_8countries/ID_lora/final_model` |
| 模型大小 | 0.02 GB |
| 可训参数 | 596,745 (0.21% of 279M) |
| 数据源 | Datasets_Nine_country/Indonesia/all_datasets_with_labels_dedup.csv |
...
|------|-----------|--------|------|--------|
| **clean (safe)** | 0.8615 | 0.7849 | **0.8214** | 2441 |
| **violation** | 0.8002 | 0.8723 | **0.8347** | 2411 |
...
- **clean recall = 0.7849**: 78.5% 的安全内容被正确识别
- **violation recall = 0.8723**: 87.2% 的违规内容被正确召回
- **clean precision = 0.8615**: 模型判为 clean 的内容中 86.2% 确实是 clean
- **violation precision = 0.8002**: 模型判为 violation 的内容中 80.0% 确实是违规
...
model = PeftModel.from_pretrained(
    base_model,
    "/home/ninini/Agents/Colloation_data/Training/binary_8countries/ID_lora/final_model"
)
...
# 示例
text = "some Indonesia social media post..."
...
| 指标 | Full FT | LoRA | Δ |
|------|---------|------|------|
| Accuracy | 0.8568 | 0.8283 | -0.0284 |
| Macro F1 | 0.8568 | 0.8281 | -0.0287 |
| Clean F1 | 0.8562 | 0.8214 | -0.0348 |
| Viol F1 | 0.8573 | 0.8347 | -0.0226 |
| 训练参数 | 279M | 597K (0.21%) | - |
| 训练速度 | 基准 | 更快 | - |
...
- Full FT 版本: [ID_fullft.md](ID_fullft.md)（更高精度）
