...
| 模型路径 | `Training/binary_8countries/SA_lora/final_model` |
| 模型大小 | 0.02 GB |
| 可训参数 | 596,745 (0.21% of 279M) |
| 数据源 | Datasets_Nine_country/Saudi Arabia/all_datasets_with_labels_dedup.csv |
...
|------|-----------|--------|------|--------|
| **clean (safe)** | 0.8223 | 0.7681 | **0.7943** | 4830 |
| **violation** | 0.8443 | 0.8833 | **0.8634** | 6875 |
...
- **clean recall = 0.7681**: 76.8% 的安全内容被正确识别
- **violation recall = 0.8833**: 88.3% 的违规内容被正确召回
- **clean precision = 0.8223**: 模型判为 clean 的内容中 82.2% 确实是 clean
- **violation precision = 0.8443**: 模型判为 violation 的内容中 84.4% 确实是违规
...
model = PeftModel.from_pretrained(
    base_model,
    "/home/ninini/Agents/Colloation_data/Training/binary_8countries/SA_lora/final_model"
)
...
# 示例
text = "some Saudi Arabia social media post..."
...
| 指标 | Full FT | LoRA | Δ |
|------|---------|------|------|
| Accuracy | 0.8771 | 0.8358 | -0.0413 |
| Macro F1 | 0.8736 | 0.8288 | -0.0448 |
| Clean F1 | 0.8524 | 0.7943 | -0.0581 |
| Viol F1 | 0.8948 | 0.8634 | -0.0314 |
| 训练参数 | 279M | 597K (0.21%) | - |
| 训练速度 | 基准 | 更快 | - |
...
- **仅二分类**: 不能区分违规类型
- **单国家**: 仅针对 Saudi Arabia
- **精度低于 Full FT**: 适合快速部署，生产环境建议 Full FT

## 相关模型

- Full FT 版本: [SA_fullft.md](SA_fullft.md)（更高精度）
