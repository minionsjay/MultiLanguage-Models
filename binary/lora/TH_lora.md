...
| 模型路径 | `Training/binary_8countries/TH_lora/final_model` |
| 模型大小 | 0.02 GB |
| 可训参数 | 596,745 (0.21% of 279M) |
| 数据源 | Datasets_Nine_country/Thailand/all_datasets_with_labels_dedup.csv |
...
|------|-----------|--------|------|--------|
| **clean (safe)** | 0.7907 | 0.7736 | **0.7821** | 879 |
| **violation** | 0.8676 | 0.8787 | **0.8731** | 1484 |
...
- **clean recall = 0.7736**: 77.4% 的安全内容被正确识别
- **violation recall = 0.8787**: 87.9% 的违规内容被正确召回
- **clean precision = 0.7907**: 模型判为 clean 的内容中 79.1% 确实是 clean
- **violation precision = 0.8676**: 模型判为 violation 的内容中 86.8% 确实是违规
...
model = PeftModel.from_pretrained(
    base_model,
    "/home/ninini/Agents/Colloation_data/Training/binary_8countries/TH_lora/final_model"
)
...
# 示例
text = "some Thailand social media post..."
...
| 指标 | Full FT | LoRA | Δ |
|------|---------|------|------|
| Accuracy | 0.8747 | 0.8396 | -0.0351 |
| Macro F1 | 0.8675 | 0.8276 | -0.0399 |
| Clean F1 | 0.8366 | 0.7821 | -0.0546 |
| Viol F1 | 0.8984 | 0.8731 | -0.0253 |
| 训练参数 | 279M | 597K (0.21%) | - |
| 训练速度 | 基准 | 更快 | - |
...
- Full FT 版本: [TH_fullft.md](TH_fullft.md)（更高精度）
