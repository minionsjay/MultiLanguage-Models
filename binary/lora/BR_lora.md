...
| 模型路径 | `Training/binary_8countries/BR_lora/final_model` |
| 模型大小 | 0.02 GB |
| 可训参数 | 596,745 (0.21% of 279M) |
| 数据源 | Datasets_Nine_country/Brazil/all_datasets_with_labels_dedup.csv |
...
|------|-----------|--------|------|--------|
| **clean (safe)** | 0.9457 | 0.8205 | **0.8787** | 9868 |
| **violation** | 0.6075 | 0.8550 | **0.7103** | 3206 |
...
- **clean recall = 0.8205**: 82.1% 的安全内容被正确识别
- **violation recall = 0.8550**: 85.5% 的违规内容被正确召回
- **clean precision = 0.9457**: 模型判为 clean 的内容中 94.6% 确实是 clean
- **violation precision = 0.6075**: 模型判为 violation 的内容中 60.7% 确实是违规
...
model = PeftModel.from_pretrained(
    base_model,
    "/home/ninini/Agents/Colloation_data/Training/binary_8countries/BR_lora/final_model"
)
...
# 示例
text = "some Brazil social media post..."
...
| 指标 | Full FT | LoRA | Δ |
|------|---------|------|------|
| Accuracy | 0.8652 | 0.8290 | -0.0362 |
| Macro F1 | 0.8288 | 0.7945 | -0.0343 |
| Clean F1 | 0.9077 | 0.8787 | -0.0290 |
| Viol F1 | 0.7500 | 0.7103 | -0.0397 |
| 训练参数 | 279M | 597K (0.21%) | - |
| 训练速度 | 基准 | 更快 | - |
...
- Full FT 版本: [BR_fullft.md](BR_fullft.md)（更高精度）
